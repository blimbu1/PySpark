...
for k in data1:
	...
	print "Total: %d" %k[1]
	noe = float(k[1])/counted[k[0]]
	print "Percentage: %0.2f" %noe
	print "grade: %s" %(grades(noe))
	if noe > maxi:
		max_id = k[0]
		maxi = noe
	print "--------------------------------------------------------------"

print "Max percentage is %0.2f" %maxi
print "The person's id is: %s" %max_id
for k in data2:
	if k[0]==max_id:
		print "Name of Student with highest percentage: %s" %k[1]
		print "Address of Student with highest percentage: %s" %k[2]
